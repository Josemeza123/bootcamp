        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('chrips.store') }}" method="post">
                        @csrf
                        <textarea name="message" id="" cols="30" rows="10" placeholder="{{ __('What\'s happening?') }}"
                            class="block w-full rounded-md border-gray-300 bg-white shadow-sm transition-colors duration-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:border-indigo-300 dark:focus:ring dark:focus:ring-indigo-200 dark:focus:ring-opacity-50">{{ old('message') }}</textarea>
                            ...
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    @if ($chirp->created_at != $chirp->updated_at)
                                        <small
                                            class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $chirp->updated_at->format('j M Y, g:i a') }}</small>
                                        <small
                                            class="text-sm text-gray-600 dark:text-gray-400">&middot;{{ __('edit') }}</small>
                                    @endif
                                </div>
                            </div>
